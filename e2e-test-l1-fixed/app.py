"""
e2e-test-l1-fixed - Chatbot Web App
...
llm = DatabricksLLM(
    endpoint=config.get("model", {}).get("endpoint", "databricks-claude-sonnet-4-5"),
    auto_trace=config.get("mlflow", {}).get("auto_trace", True)
)

# Set up MLflow tracking
mlflow.set_tracking_uri("databricks")
experiment = config.get("mlflow", {}).get("experiment", "/Shared/e2e-test-l1-fixed")
try:
...
<body>
    <h1>🤖 e2e-test-l1-fixed</h1>
    <p class="subtitle">Powered by <strong>databricks-claude-sonnet-4-5</strong></p>
...
def home():
    ...
    return render_template_string(
        CHAT_HTML,
        name=config.get("agent_name", "e2e-test-l1-fixed"),
        model=config.get("model", {}).get("endpoint", "databricks-claude-sonnet-4-5")
    )
...
def chat():
    ...
    try:
        ...
        # Call LLM (MLflow tracing happens automatically)
        import asyncio
...
if __name__ == '__main__':
    print(f"🚀 Starting e2e-test-l1-fixed on port {PORT}", flush=True)
