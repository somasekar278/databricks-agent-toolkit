...
4. **Start the app:**
```bash
databricks bundle run e2e-test-l1-fixed
```

5. **Add Model Serving permission (one-time setup):**
   
   After first deployment, add Model Serving access:
   - Go to **Databricks Workspace** → **Apps** → **e2e-test-l1-fixed**
   - Click **Resources** → **Add Resource**  
   - Add **Model Serving** with **Can query** permission
   - Then start: `databricks bundle run e2e-test-l1-fixed`

6. **Access your app:**
   `https://your-workspace.cloud.databricks.com/ml/apps/e2e-test-l1-fixed`

**Deploy to production:**
```bash
databricks bundle deploy -t prod
databricks bundle run e2e-test-l1-fixed -t prod
```
...
```bash
databricks-agent-toolkit generate assistant e2e-test-l1-fixed-with-memory
```
