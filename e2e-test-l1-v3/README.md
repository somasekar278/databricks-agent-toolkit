# e2e-test-l1-v3 - L1 Simple Chatbot
...
4. **Start the app:**
```bash
databricks bundle run e2e-test-l1-v3
```

5. **Add Model Serving permission (one-time setup):**
   
   After first deployment, add Model Serving access:
   - Go to **Databricks Workspace** → **Apps** → **e2e-test-l1-v3**
   - Click **Resources** → **Add Resource**  
   - Add **Model Serving** with **Can query** permission
   - Then start: `databricks bundle run e2e-test-l1-v3`

6. **Access your app:**
   `https://your-workspace.cloud.databricks.com/ml/apps/e2e-test-l1-v3`

**Deploy to production:**
```bash
databricks bundle deploy -t prod
databricks bundle run e2e-test-l1-v3 -t prod
```
...
```bash
databricks-agent-toolkit generate assistant e2e-test-l1-v3-with-memory
```
