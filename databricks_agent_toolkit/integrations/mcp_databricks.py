        ...
        for name, config in servers.items():
            if name == "vector_search":
                ...
                if not catalog or not schema:
                    raise ValueError(f"vector_search requires 'catalog' and 'schema'")
                endpoints[name] = f"{self.host}/api/2.0/mcp/vector-search/{catalog}/{schema}"
            
            elif name == "genie":
                space_id = config.get("space_id")
                if not space_id:
                    raise ValueError(f"genie requires 'space_id'")
                ...
                schema = config.get("schema")
                if not catalog or not schema:
                    raise ValueError(f"uc_functions requires 'catalog' and 'schema'")
