# e2e-test-l1-final - L1 Simple Chatbot
...
4. **Start the app:**
```bash
databricks bundle run e2e-test-l1-final
```

5. **Add Model Serving permission (one-time setup):**
   
   After first deployment, add Model Serving access:
   - Go to **Databricks Workspace** → **Apps** → **e2e-test-l1-final**
   - Click **Resources** → **Add Resource**  
   - Add **Model Serving** with **Can query** permission
   - Then start: `databricks bundle run e2e-test-l1-final`

6. **Access your app:**
   `https://your-workspace.cloud.databricks.com/ml/apps/e2e-test-l1-final`

**Deploy to production:**
```bash
databricks bundle deploy -t prod
databricks bundle run e2e-test-l1-final -t prod
```
...
```yaml
endpoint: databricks-claude-sonnet-4-5  # Try different models
```

**Available models:**
- `databricks-claude-sonnet-4-5` - High quality, recommended ⭐
...
**Use when:**
- ✅ You need the full response before displaying
- ✅ Processing the response (parsing, validation)
- ✅ Storing complete responses in logs
...
- ✅ Automatic MLflow tracing (`@mlflow.trace`)
...
## 📊 View Your Traces & Provide Feedback
...
4. **Assessments** - Your feedback ratings ⭐
...
```
📊 Quick Feedback (helps improve the bot)
==================================================
...
- ✅ Identifies edge cases & problems
- ✅ Triggers optimization when quality drops
- ✅ Provides ground truth for scorer tuning
...
```
User Conversations → Traces → Assessments → Dataset → Evaluation → Optimization → Deploy
                      ↑                                                              ↓
                      └──────────────────────────────────────────────────────────────┘
```
...
3. **Dataset curated** - High/low-rated examples → training data
...
```bash
databricks-agent-toolkit generate assistant e2e-test-l1-final-with-memory
```

**Key differences:**
- ✅ Stores conversation history in Lakebase
...
**Generated with ❤️ by Databricks Agent Toolkit**
